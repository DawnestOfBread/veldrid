...
namespace Veldrid.ImageSharp
{
    public class ImageSharpCubemapTexture
    {
        ...
        public unsafe Texture CreateDeviceTexture(GraphicsDevice gd, ResourceFactory factory)
        {
            ...
            {
                if (!CubemapTextures[PositiveXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelMemPosX))
                {
                    throw new VeldridException("Unable to get positive x pixelspan.");
                }
                if (!CubemapTextures[NegativeXArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelMemNegX))
                {
                    throw new VeldridException("Unable to get negatve x pixelspan.");
                }
                if (!CubemapTextures[PositiveYArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelMemPosY))
                {
                    throw new VeldridException("Unable to get positive y pixelspan.");
                }
                if (!CubemapTextures[NegativeYArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelMemNegY))
                {
                    throw new VeldridException("Unable to get negatve y pixelspan.");
                }
                if (!CubemapTextures[PositiveZArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelMemPosZ))
                {
                    throw new VeldridException("Unable to get positive z pixelspan.");
                }
                if (!CubemapTextures[NegativeZArrayLayer][level].DangerousTryGetSinglePixelMemory(out Memory<Rgba32> pixelMemNegZ))
                {
                    throw new VeldridException("Unable to get negatve z pixelspan.");
                }
                fixed (Rgba32* positiveXPin = &MemoryMarshal.GetReference(pixelMemPosX.Span))
                fixed (Rgba32* negativeXPin = &MemoryMarshal.GetReference(pixelMemNegX.Span))
                fixed (Rgba32* positiveYPin = &MemoryMarshal.GetReference(pixelMemPosY.Span))
                fixed (Rgba32* negativeYPin = &MemoryMarshal.GetReference(pixelMemNegY.Span))
                fixed (Rgba32* positiveZPin = &MemoryMarshal.GetReference(pixelMemPosZ.Span))
                fixed (Rgba32* negativeZPin = &MemoryMarshal.GetReference(pixelMemNegZ.Span))
                {
